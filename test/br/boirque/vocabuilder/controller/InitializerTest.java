...
public class InitializerTest extends TestCase implements TestConstants {
	...
	public void testLoadUniqueSetNames() throws IOException {
		...
		for (int i = 0; i < uniqueSets.length; i++) {
			...
			assertEquals("wrong NewUnique count", DEFAULTSETCOUNT,
					newUniqueSets.length);
			// number of default sets remain intact
			String[] newDefaultSets = init.loadDefaultSetNames();
			assertNotNull(newDefaultSets);
			assertEquals("wrong default count", DEFAULTSETCOUNT,
					newDefaultSets.length);
	...
	/**
	 * the testSet has 8 cards we changed the amount for the first 3 of them.
	 * The last 5 cards should have 0 viewings card 0 and card 1 must have 1
	 * viewing each and card 2 must have 30 viewings. additionally, the least
	 * viewed cards must appear first
	 * 
	 * @param init
	 * @param cards
	 */
	private void assertCardViewCountAndOrder(Initializer init, Vector cards) {
		...
		for (int i = 0; i < 5; i++) {
			...
			assertTrue("viewedCounter must be 0 for card " + index + " but is "
					+ card.getViewedCounter(), card.getViewedCounter() == 0);
			// correct indexes?
		...
		for (int i = 0; i < 2; i++) {
			...
			assertTrue("viewedCounter must be 1 for card " + index + " but is "
					+ card.getViewedCounter(), card.getViewedCounter() == 1);
			assertTrue(index == 0 || index == 1);
		}

		int index = init.getNextCardIndex(cards);
		FlashCard card = (FlashCard) cards.elementAt(index);
		assertTrue("viewedCounter must be 30 for card " + index + " but is "
				+ card.getViewedCounter(), card.getViewedCounter() == 30);
		assertTrue(index == 2);
	...
	public void testDownloadSetOfCards() {
		Initializer init = new Initializer();
		SetOfCards result = init.downloadSetOfCards("1");
		assertNotNull("Set is null", result);
		assertNotNull("Inner card vector is null", result.getFlashCards() );
		assertEquals("Wrong card amount", 50, result.getFlashCards().size());
	}

	public Test suite() {
		TestSuite testsuite = new TestSuite();

		testsuite.addTest(new InitializerTest("testLoadSet", new TestMethod() {
			public void run(TestCase tc) throws RecordStoreNotOpenException,
					RecordStoreFullException, IOException, RecordStoreException {
		...
		testsuite.addTest(new InitializerTest("testDownloadSetOfCards",
				new TestMethod() {
					public void run(TestCase tc) throws IOException {
						((InitializerTest) tc).testDownloadSetOfCards();
					}
				}));
